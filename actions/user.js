    ...
    const result = await db.$transaction(
      async (tx) => {
        ...
        if (!industryInsight) {
          ...
          // Ensure demandLevel is valid, otherwise set to "MEDIUM"
          if (!validDemandLevels.includes(insights.demandLevel)) {
            console.warn(`Invalid demandLevel received: ${insights.demandLevel}, defaulting to MEDIUM`);
            insights.demandLevel = "MEDIUM";
          }

          // Ensure marketOutlook is valid, otherwise set to "NEUTRAL"
          if (!validMarketOutlooks.includes(insights.marketOutlook)) {
            console.warn(`Invalid marketOutlook received: ${insights.marketOutlook}, defaulting to NEUTRAL`);
